...
def predictions_for_one_voxel(voxel_path, top_N):
    ...
    text_inputs = torch.cat([clip.tokenize(f"a volume data of a {c}") for c in classes]).to(device)#torch.Size([100, 77])
...
def main():######################################################################################################
    ...
    if args.mode == "train":
        ...
        # Load the model
        global device, model, preprocess, classes
        device = "cuda" if torch.cuda.is_available() else "cpu"
        model, preprocess = clip.load('ViT-B/32', device)
        classes = get_all_folder_names('data/ModelNet40')#[ "piano", "bathtub","bench","bookshelf", "bottle","xbox"]
